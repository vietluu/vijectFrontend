  ...
  const handleUpload = async () => {
    ...
    try {
      if (fileList.length) {
        ...
        if (res.status === 201) {
          dispatch(updateProfile({ image: res.data.image_url })).then((res) => {
      ...
      >
        <Upload
          ...
          beforeUpload={(file) => {
            ...
            if (!isImg) {
              message.error(`${file.name} không phải là định dạng cho phép`);
            }
            return false;
          }}
          onRemove={(file) => {
        ...
        >
          {fileList.length >= 1 ? null : (
            <div className="flex aspect-square h-44 items-center justify-center dark:text-white mobile:h-48">
              ...
              <div className="text-black text-center">Chọn ảnh</div>
            </div>
          )}
        </Upload>
      </Modal>
